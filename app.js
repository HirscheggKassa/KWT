...
// DEXIE (Lokaler Cache)
// ================================
const db = new Dexie(APP_CONFIG.app.db_name);
...
const i18n = {
    ...
    // Button HTML rendern - oben mitte mit Flaggen (nur Sprache) - STICKY VERSION
    renderLangButton() {
        const flag = this.currentLang === 'de' ? '🇬🇧' : '🇩🇪';
        const label = this.currentLang === 'de' ? 'English' : 'Deutsch';
        return `
        <div style="position:sticky;top:0;left:0;right:0;z-index:1000;background:linear-gradient(135deg,#f8f9fa,#e9ecef);padding:12px 16px;display:flex;justify-content:center;align-items:center;gap:10px;box-shadow:0 2px 12px rgba(0,0,0,0.08);border-bottom:1px solid rgba(44,95,124,0.1);">
...
const RegisteredGuests = {
    ...
    async register(firstName, password) {
        ...
        if (supabaseClient && isOnline) {
            ...
            // Lokalen Cache aktualisieren - PIN wird gehasht gespeichert
            const pinHash = await Utils.hashPassword(password);
            const localGuest = { 
                id: userId, 
                firstName: cleanName,
                nachname: cleanName,
            ...
            // Offline-Modus: Lokal speichern - PIN wird gehasht
            const offlinePinHash = await Utils.hashPassword(password);
            const guest = { 
                id: Utils.uuid(),
                firstName: cleanName, 
                nachname: cleanName,
                passwort: offlinePinHash,
                passwordHash: offlinePinHash,
                gruppenname: 'keiner Gruppe zugehörig',
                group_name: 'keiner Gruppe zugehörig',
                ausnahmeumlage: false,
                aktiv: true,
...
const Buchungen = {
    ...
    async storno(buchung_id) {
        ...
        console.log('🔄 Storniere Buchung:', buchung_id);
        ...
        console.log('📋 Original-Buchung:', {
            id: buchung_id,
            artikel: b.artikel_name,
            menge: b.menge,
            preis: b.preis,
            exportiert: b.exportiert
        });
        ...
        console.log('✅ Buchung als storniert markiert');
        
        // =====================================================
        // 2. GEGENBUCHUNG ERSTELLEN (wenn bereits exportiert)
        // =====================================================
        if (b.exportiert === true) {
            console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            console.log('📤 Buchung war bereits exportiert!');
            console.log('💡 Erstelle Gegenbuchung für automatischen IDEAS-Import...');
            console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            
            // Gegenbuchung mit NEGATIVEN Werten
            const gegenbuchung = {
                buchung_id: `STORNO_${buchung_id}`,
                user_id: b.user_id,
                gast_id: b.gast_id,
                gast_vorname: b.gast_vorname,
                gast_nachname: b.gast_nachname,
                artikel_id: b.artikel_id,
                artikel_name: b.artikel_name,
                kategorie_id: b.kategorie_id,
                menge: -(b.menge || 1),           // ⚠️ NEGATIV!
                preis: -(b.preis || 0),           // ⚠️ NEGATIV!
                steuer_prozent: b.steuer_prozent,
                datum: new Date().toISOString().split('T')[0],
                uhrzeit: new Date().toTimeString().split(' ')[0].substring(0, 5),
                erstellt_am: new Date().toISOString(),
                group_name: b.group_name,
                gastgruppe: b.gastgruppe,
                session_id: b.session_id,
                fix: b.fix,
                storniert: false,                 // ⚠️ WICHTIG: false, damit IDEAS es sieht!
                exportiert: false                 // ⚠️ WICHTIG: false, damit IDEAS es abholt!
            };
            
            console.log('📝 Gegenbuchung:', {
                id: gegenbuchung.buchung_id,
                artikel: gegenbuchung.artikel_name,
                menge: gegenbuchung.menge,
                preis: gegenbuchung.preis,
                storniert: gegenbuchung.storniert,
                exportiert: gegenbuchung.exportiert
            });
            ...
            if (insertError) {
                console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
                console.error('❌ FEHLER beim Erstellen der Gegenbuchung!');
                console.error('Fehler:', insertError);
                console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
                console.warn('⚠️ Original-Buchung wurde storniert,');
                console.warn('⚠️ aber Gegenbuchung konnte nicht erstellt werden!');
                console.warn('⚠️ Artikel bleibt auf IDEAS-Registrierkasse!');
            } else {
                console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
                console.log('✅ Gegenbuchung erfolgreich erstellt!');
                console.log('🚀 IDEAS wird sie beim nächsten Auto-Sync (alle 30 Sek) abholen!');
                console.log('📊 Artikel wird automatisch von Registrierkasse entfernt!');
                console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            }
        } else {
...
const Gruppen = {
    ...
    async setAbfrageAktiv(aktiv) {
        ...
        // Dann in Supabase speichern (falls online)
        if (supabaseClient && isOnline) {
            try {
    ...
    async getAll() {
        if (supabaseClient && isOnline) {
            try {
                ...
                if (error) {
                    console.warn('Gruppen laden Supabase Fehler:', error);
                } else if (data && data.length > 0) {
                    console.log('✅ Gruppen von Supabase geladen:', data.length);
                    // Lokal cachen
                    for (const g of data) {
        ...
        // Fallback: Lokal - aktiv kann boolean oder number sein
        const allLocal = await db.gruppen.toArray();
        const active = allLocal.filter(g => g.aktiv === true || g.aktiv === 1);
        console.log('Gruppen aus lokalem Cache:', active.length);
        return active;
    },
...
// PreisModus - Vereinfacht (kein HP/SV Dual-Pricing)
const PreisModus = {
    SELBSTVERSORGER: 'sv',
    HP: 'hp',
    async getModus() { return 'sv'; },
    async setModus() {},
    getModusText() { return 'Standard'; },
    getPreis(artikel) { return artikel.preis ?? 0; },
    getBeidePreise(artikel) { return { sv: artikel.preis ?? 0, hp: artikel.preis ?? 0 }; },
...
Router.register('admin-alle-buchungen', async () => {
    ...
    const datumVon = State.buchungenDatumVon || '';
    const datumBis = State.buchungenDatumBis || '';
    ...
    if (datumVon) {
        bs = bs.filter(b => b.datum >= datumVon);
    }
    if (datumBis) {
        bs = bs.filter(b => b.datum <= datumBis);
    }
    ...
    <div class="main-content">
            ...
            <div style="padding:16px;">
                <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:end;">
                    <div style="flex:1;min-width:180px;">
                        <label style="font-weight:600;font-size:0.85rem;display:block;margin-bottom:4px;">Gast:</label>
                    ...
                    <div style="min-width:140px;">
                        <label style="font-weight:600;font-size:0.85rem;display:block;margin-bottom:4px;">Von:</label>
                        <input type="date" id="datum-von" class="form-input" value="${datumVon}" onchange="filterBuchungenByDatum()" style="padding:12px;font-size:0.95rem;">
                    </div>
                    <div style="min-width:140px;">
                        <label style="font-weight:600;font-size:0.85rem;display:block;margin-bottom:4px;">Bis:</label>
                        <input type="date" id="datum-bis" class="form-input" value="${datumBis}" onchange="filterBuchungenByDatum()" style="padding:12px;font-size:0.95rem;">
                    </div>
                    <div>
                        <button class="btn btn-secondary" onclick="resetBuchungenFilter()" style="padding:12px 16px;font-size:0.9rem;">✕ Reset</button>
                    </div>
                    ${selectedGastId ? `<button class="btn btn-primary" onclick="showAddBuchungForGastModal('${selectedGastId}', '${selectedGastName}')" style="padding:12px 20px;">+ Buchung</button>` : ''}
                </div>
            </div>
        </div>
        ...
        <div class="card mb-3" style="background:var(--color-alpine-green);color:white;">
            <div style="padding:16px;text-align:center;">
                ...
                <div>${selectedGastId ? selectedGastName : 'Alle Gäste'} | Summe: ${Utils.formatCurrency(gesamtSumme)}${datumVon || datumBis ? ` | ${datumVon || '...'} bis ${datumBis || '...'}` : ''}</div>
            </div>
        </div>
                    ...
                    <tbody>
                        ${bs.length ? bs.map(b => `
                            <tr style="border-bottom:1px solid var(--color-stone-medium);${b.storniert ? 'opacity:0.5;text-decoration:line-through;' : ''}">
                                <td style="padding:8px;white-space:nowrap;font-size:0.8rem;">${b.datum || '-'}</td>
                                <td style="padding:8px;font-size:0.8rem;">${b.uhrzeit || '-'}</td>
                                <td style="padding:8px;font-weight:500;">${b.gast_vorname || '?'}</td>
                                <td style="padding:8px;">${b.artikel_name || '-'}</td>
                                <td style="padding:8px;text-align:right;">${b.menge}x</td>
                                <td style="padding:8px;text-align:right;font-weight:600;">${Utils.formatCurrency(b.preis * b.menge)}</td>
                                <td style="padding:8px;text-align:center;">
                                    ${b.storniert 
                                        ? '<span style="color:#e74c3c;font-size:0.75rem;">Storno</span>'
                                        : `<button class="btn btn-danger" style="padding:4px 10px;font-size:0.75rem;" onclick="handleAdminDeleteBuchung('${b.buchung_id}')">🗑</button>`
                                    }
                                </td>
                            </tr>
                        `).join('') : '<tr><td colspan="7" style="padding:40px;text-align:center;color:#888;">Keine Buchungen</td></tr>'}
                    </tbody>
                </table>
            </div>
        </div>
...
Router.register('admin-alte-belege', async () => {
    if (!State.isAdmin) { Router.navigate('admin-login'); return; }
    let aktiveGäste = []; let inaktiveGäste = [];
    if (supabaseClient && isOnline) {
        try { const { data } = await supabaseClient.from('profiles').select('id, vorname, first_name, display_name, group_name').eq('aktiv', true).eq('geloescht', false).order('vorname'); if (data) aktiveGäste = data.map(g => ({ id: g.id, name: g.display_name || g.vorname || g.first_name || '?', gruppe: g.group_name || '-' })); } catch(e) {}
    ...
    const seenIds = new Set(); aktiveGäste = aktiveGäste.filter(g => { if (seenIds.has(g.id)) return false; seenIds.add(g.id); return true; }); inaktiveGäste = inaktiveGäste.filter(g => { if (seenIds.has(g.id)) return false; seenIds.add(g.id); return true; });
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">←</button><div class="header-title">📋 Belege</div></div><div class="header-right"><button class="btn btn-secondary" onclick="Router.navigate('admin-dashboard')">← Dashboard</button></div></div>
    <div class="main-content">
        <div class="card mb-3" style="background:linear-gradient(135deg,#636e72,#2d3436);color:white;"><div style="padding:16px;text-align:center;"><div style="font-size:1.3rem;font-weight:700;">📋 Belege drucken</div><div style="opacity:0.9;font-size:0.9rem;">${aktiveGäste.length} aktive • ${inaktiveGäste.length} ausgecheckte Gäste</div></div></div>
        <div class="card mb-3" style="background:#fffde7;"><div style="padding:16px;"><label style="font-weight:600;display:block;margin-bottom:8px;">Gast auswählen:</label><select id="alte-belege-gast" class="form-input" style="width:100%;padding:14px;font-size:1.1rem;" onchange="selectAlteBelegeGast(this.value)"><option value="">-- Gast wählen --</option><optgroup label="🟢 Aktive Gäste">${aktiveGäste.map(g => `<option value="${g.id}" ${g.id === selectedId ? 'selected' : ''}>${g.name} (${g.gruppe})</option>`).join('')}</optgroup><optgroup label="🔴 Ausgecheckte Gäste">${inaktiveGäste.map(g => `<option value="${g.id}" ${g.id === selectedId ? 'selected' : ''}>${g.name} (${g.gruppe})</option>`).join('')}</optgroup></select><input type="text" class="form-input" placeholder="🔍 Suchen..." oninput="filterAlteBelegeDropdown(this.value)" style="width:100%;padding:12px;margin-top:8px;"></div></div>
...
window.druckeBeleg = async (gastId, gastName, gesamtSumme) => {
    ...
    // MwSt Berechnung: 10% auf Speisen, 20% auf Getränke (Österreich)
    // Vereinfacht: Alles mit 20% MwSt (Getränke-Pauschale für Kassa)
    const mwstSatz = 20;
    const netto = gesamtSumme / (1 + mwstSatz / 100);
    const mwstBetrag = gesamtSumme - netto;
    const byD={}; buchungen.forEach(b=>{const d=b.datum||'?';if(!byD[d])byD[d]=[];byD[d].push(b);});
    const fmt=(v)=>new Intl.NumberFormat('de-AT',{style:'currency',currency:'EUR'}).format(v||0);
    // Thermodrucker-optimiertes Format (80mm Bonbreite)
...
window.filterBuchungenByDatum = () => {
    State.buchungenDatumVon = document.getElementById('datum-von')?.value || '';
    State.buchungenDatumBis = document.getElementById('datum-bis')?.value || '';
    Router.navigate('admin-alle-buchungen');
};

window.resetBuchungenFilter = () => {
...
window.toggleGruppenAbfrage = async (aktiv) => {
    console.log('toggleGruppenAbfrage aufgerufen mit:', aktiv);
    
    try {
        if (aktiv) {
            const gruppen = await Gruppen.getAll();
            console.log('Gruppen gefunden:', gruppen?.length || 0);
            if (!gruppen || gruppen.length === 0) {
        ...
        setTimeout(() => Router.navigate('admin-gruppen'), 300);
    } catch(e) {
        console.error('toggleGruppenAbfrage Fehler:', e);
        Utils.showToast('❌ Fehler: ' + e.message, 'error');
        // Toggle zurücksetzen
...
(async function initApp() {
    console.log(` ${APP_CONFIG.betrieb.name} ${APP_CONFIG.betrieb.kurzname} ${APP_CONFIG.betrieb.version} (Supabase) startet...`);
